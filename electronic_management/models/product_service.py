...
class ProductService(models.Model):
    ...
    customer_name = fields.Many2one(string="Customer Name", related='invoice.partner_id')
    invoice_date = fields.Date("Invoice Date", related="invoice.invoice_date", readonly=True)
    product = fields.Many2one(string="Product Name", related="invoice.invoice_line_ids.product_id", readonly=True)
    phone = fields.Char(string="Contact Number", related="customer_name.phone")
    invoice_currency_id = fields.Many2one(string="Currency ID", related="invoice.currency_id")
    invoice_price = fields.Float(string="Product Price", related="invoice.invoice_line_ids.price_unit", readonly=True)
    fault = fields.Text(string="Fault in product")
    service_date = fields.Date(string='Service Date', default=datetime.today(), readonly=True)
    ...
    def whatsapp_invoice(self):
        message = 'Hi %s, we received your service request for product %s has fault of %s.Please collect your product on %s'% (self.customer_name.name,self.product.name,self.fault,self.pickup_date)
        ...
        for rec in self:
            if rec.pickup_date:
                if rec.pickup_date <= rec.service_date:
                    raise ValidationError(_('Pickup Date Must be greater than Today Date...'))
